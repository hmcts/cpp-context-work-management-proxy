...
public class HearingDateComparatorTest extends BaseSortingTest {

    @Test
    public void testHearingDateSorting() {

        final List<Task> tasks = createProcessInstanceTaskVariablesWithHearingDates();
        ...
        List<TaskWithVariables> taskWithVariables = tasks.stream().map(tk -> {
            ...
            return ts;
        }).sorted(hearingDateComparator).collect(toList());
    ...
    private void prettyPrint(final List<TaskWithVariables> taskWithVariables) {
        LOGGER.info(String.format("%-20s%-30s%-30s\n", "Task Name", "HEARING DATE", "CREATE DATE"));
        for (TaskWithVariables tasks : taskWithVariables) {
            ...
            LOGGER.info(String.format("%-20s%-30s%-30s\n", tasks.getTask().getName(), varMap.get(HEARING_DATE), tasks.getTask().getCreateTime()));
        }
    }

    private List<Task> createProcessInstanceTaskVariablesWithHearingDates() {
